...
LOG_MODULE_REGISTER(is31fl3743a, CONFIG_LED_STRIP_LOG_LEVEL);
...
int static is31fl3743a_init(const struct device *dev) {
    ...
    // If available, pull SDB high
    if (gpio_is_ready_dt(&config->gpio)) {
        if (gpio_pin_configure_dt(&config->gpio, GPIO_OUTPUT)) {
            LOG_ERR("SDB pin for is31fl3743a@%x cannot be configured", config->i2c.addr);
            return -EIO;
        }

        if (gpio_pin_set_dt(&config->gpio, 1)) {
            LOG_ERR("SDB pin for is31fl3743a@%x cannot be pulled high", config->i2c.addr);
            return -EIO;
        }
    } else {
        LOG_WRN("%s: sdb pin not configured", dev->name);
    }
    ...
    is31fl3743a_reg_burst_write(dev, 0x01, px_buffer, config->px_buffer_size);
    ...
    LOG_INF("IS31FL3743A INIT COMPLETE");
...
// The device is configurable for a 18xn matrix where: 1 <= n <= 11
#define IS31FL3743A_BUFFER_SIZE(idx)                                                               \
    18 * (11 - DT_INST_PROP(idx, sw_setting))
...
#define IS31FL3743A_DEVICE(idx)                                                                    \
    ...
    static const struct is31fl3743a_config is31fl3743a_##idx##_config = {                          \
        ...
        .gpio = GPIO_DT_SPEC_INST_GET_OR(idx, sdb_gpios, {0}),                                      \
